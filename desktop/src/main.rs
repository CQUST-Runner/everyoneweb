...
fn close_server() {
    ...
    let result = sys.processes_by_name("webbook-server");
    for p in result {
        // TODO: more gentle
        p.kill();
    }
...
fn start_server(exe_path: PathBuf, p: &PathResolver) {
    ...
    cmd = cmd
        ...
        .arg("-log")
        .arg(log_dir.unwrap().join("webbook-server.log"))
        .arg("-id")
        .arg(current_id)
...
fn handle_menu_event(event: WindowMenuEvent) {
    match event.menu_item_id() {
        ...
        "LearnMore" => {
            let _ = open(
                &event.window().app_handle().shell_scope(),
                "https://github.com/CQUST-Runner/webbook",
                None,
            );
        }
        _ => {}
    ...
    tauri::Builder::default()
        ...
        .setup(|app| {
            ...
            if server.is_some() {
                ...
                if cfg!(windows) {
                    start_server(server_path.join("webbook-server.exe"), &r);
                } else if cfg!(unix) {
                    start_server(server_path.join("webbook-server"), &r);
                }
                check_server_is_on();
            }
